...
class CustomConfig:
    ...
    vocab_size: int = 50304 # initial gpt2 vocab size (number of tokens: 50,000 BPE merges + 256 bytes tokens + 1 <|endoftext|> token), increasing vocab size to a  ice number (power of 2) so it works faster.
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        y = F.scaled_dot_product_attention(q, k, v, is_causal=True) # flash attention, almost 7 times faster than normal attention, uses fused kernels.
...
class GPT(nn.Module):
    ...
    def forward(self, x,y=None):
        ...
        # logits.view(-1) will flatten out B x T to single B*T , cross_entropy input will be (B*T,vocab_size), (B*T)
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device_type,master_process):
        ...
        num_decay_params = sum(p.numel() for p in decay_params) # numel returns total number of parameters in p
